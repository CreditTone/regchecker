...
//		prefs.put("profile.managed_default_content_settings.images", 2); // 禁止下载加载图片
	...
	public static Set<Cookie> getBenlaiCookies() {
		Set<Cookie> result = null;
		ChromeOptions options = new ChromeOptions();
		System.out.println(System.getProperty("os.name").toLowerCase());
		if (System.getProperty("os.name").toLowerCase().indexOf("linux") >= 0) {
...
//		prefs.put("profile.managed_default_content_settings.images", 2); // 禁止下载加载图片
	...
	public static ChromeDriver getChromeDriver(boolean disableLoadImage,HttpsProxy proxy,String userAgent) throws Exception {
		ChromeOptions options = new ChromeOptions();
		if (System.getProperty("os.name").toLowerCase().indexOf("linux") >= 0) {
			options.setBinary("/usr/bin/google-chrome");
		}
		...
		try {
			mDriver = new ChromeDriver(options) {
				@Override
				public void get(String url) {
					log.info("ChromeDriver:"+url);
					super.get(url);
					log.info("ChromeDriver Finish:"+url);
				}
				@Override
		...
		log.info("getChromeDriver:"+mDriver.getSessionId());
		return mDriver;
	}
